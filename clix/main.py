...
_clix_developer_exception_attr_name = "__clix_developer_exception__"
...
def except_hook(
    ...
    exception_config: Union[DeveloperExceptionConfig, None] = getattr(
        exc_value, _clix_developer_exception_attr_name, None
    )
    standard_traceback = os.getenv("_CLIX_STANDARD_TRACEBACK")
    if (
        standard_traceback
        ...
        return
    clix_path = os.path.dirname(__file__)
    click_path = os.path.dirname(click.__file__)
    supress_internal_dir_names = [clix_path, click_path]
    exc = exc_value
    if rich:
    ...
    for frame in tb_exc.stack:
        if any(
            ...
            if not exception_config.pretty_exceptions_short:
                # Hide the line for internal libraries, Clix and Click
                stack.append(
...
class Clix:
    def __init__(
        ...
        *,
        ...
        cls: Optional[Type[ClixGroup]] = Default(None),
        ...
        self.registered_groups: List[ClixInfo] = []
        self.registered_commands: List[CommandInfo] = []
        self.registered_callback: Optional[ClixInfo] = None
    ...
    def callback(
        ...
        *,
        ...
        def decorator(f: CommandFunctionType) -> CommandFunctionType:
            self.registered_callback = ClixInfo(
                name=name,
                cls=cls,
                invoke_without_command=invoke_without_command,
                no_args_is_help=no_args_is_help,
    ...
    def command(
        ...
        *,
        cls: Optional[Type[ClixCommand]] = None,
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            setattr(
                e,
                _clix_developer_exception_attr_name,
                DeveloperExceptionConfig(
...
def get_group(clix_instance: Clix) -> ClixGroup:
    group = get_group_from_info(
        ClixInfo(clix_instance),
        pretty_exceptions_short=clix_instance.pretty_exceptions_short,
        rich_markup_mode=clix_instance.rich_markup_mode,
    )
    return group


def get_command(clix_instance: Clix) -> click.Command:
    if clix_instance._add_completion:
        click_install_param, click_show_param = get_install_completion_arguments()
    if (
        clix_instance.registered_callback
        or clix_instance.info.callback
        or clix_instance.registered_groups
        or len(clix_instance.registered_commands) > 1
    ):
        # Create a Group
        click_command: click.Command = get_group(clix_instance)
        if clix_instance._add_completion:
            click_command.params.append(click_install_param)
            click_command.params.append(click_show_param)
        return click_command
    elif len(clix_instance.registered_commands) == 1:
        # Create a single Command
        single_command = clix_instance.registered_commands[0]

        if not single_command.context_settings and not isinstance(
            clix_instance.info.context_settings, DefaultPlaceholder
        ):
            single_command.context_settings = clix_instance.info.context_settings
        ...
        if clix_instance._add_completion:
            ...
            click_command.params.append(click_show_param)
        return click_command
    assert False, "Could not get a command for this Clix instance"  # pragma no cover


def get_group_name(clix_info: ClixInfo) -> Optional[str]:
    if clix_info.callback:
        # Priority 1: Callback passed in app.add_clix()
        return get_command_name(clix_info.callback.__name__)
    if clix_info.clix_instance:
        registered_callback = clix_info.clix_instance.registered_callback
        ...
        if clix_info.clix_instance.info.callback:
            return get_command_name(clix_info.clix_instance.info.callback.__name__)
    return None


def solve_clix_info_help(clix_info: ClixInfo) -> str:
    # Priority 1: Explicit value was set in app.add_clix()
    if not isinstance(clix_info.help, DefaultPlaceholder):
        return inspect.cleandoc(clix_info.help or "")
    # Priority 2: Explicit value was set in sub_app.callback()
    try:
        callback_help = clix_info.clix_instance.registered_callback.help
        if not isinstance(callback_help, DefaultPlaceholder):
            return inspect.cleandoc(callback_help or "")
    except AttributeError:
        pass
    # Priority 3: Explicit value was set in sub_app = clix.Clix()
    try:
        instance_help = clix_info.clix_instance.info.help
        if not isinstance(instance_help, DefaultPlaceholder):
            return inspect.cleandoc(instance_help or "")
    except AttributeError:
        pass
    # Priority 4: Implicit inference from callback docstring in app.add_clix()
    if clix_info.callback:
        doc = inspect.getdoc(clix_info.callback)
        if doc:
            return doc
    # Priority 5: Implicit inference from callback docstring in @app.callback()
    try:
        callback = clix_info.clix_instance.registered_callback.callback
        ...
        pass
    # Priority 6: Implicit inference from callback docstring in clix.Clix()
    try:
        instance_callback = clix_info.clix_instance.info.callback
    ...
    return clix_info.help.value


def solve_clix_info_defaults(clix_info: ClixInfo) -> ClixInfo:
    values: Dict[str, Any] = {}
    name = None
    for name, value in clix_info.__dict__.items():
        # Priority 1: Value was set in app.add_clix()
        ...
        try:
            callback_value = getattr(
                clix_info.clix_instance.registered_callback, name  # type: ignore
            )
    ...
    if values["name"] is None:
        values["name"] = get_group_name(clix_info)
    values["help"] = solve_clix_info_help(clix_info)
    return ClixInfo(**values)
...
def get_group_from_info(
    ...
    *,
    ...
    assert (
        group_info.clix_instance
    ), "A Clix instance is needed to generate a Click Group"
    commands: Dict[str, click.Command] = {}
    for command_info in group_info.clix_instance.registered_commands:
    ...
    for sub_group_info in group_info.clix_instance.registered_groups:
    ...
    solved_info = solve_clix_info_defaults(group_info)
    (
        params,
        convertors,
        context_param_name,
    ) = get_params_convertors_ctx_param_name_from_function(solved_info.callback)
    cls = solved_info.cls or ClixGroup
    assert issubclass(cls, ClixGroup)
    group = cls(
...
def get_command_from_info(
    ...
    *,
    ...
    cls = command_info.cls or ClixCommand
    command = cls(
        name=name,
        ...
        if origin is Union:
            ...
            for type_ in main_type.__args__:
                if type_ is NoneType:
                    continue
                types.append(type_)
            assert len(types) == 1, "Clix Currently doesn't support Union types"
            main_type = types[0]
            origin = getattr(main_type, "__origin__", None)
