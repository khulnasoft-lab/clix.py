...
class MultipleClixAnnotationsError(Exception):
    argument_name: str
    ...
    def __str__(self) -> str:
        return (
            "Cannot specify multiple `Annotated` Clix arguments"
...
def _split_annotation_from_clix_annotations(
    base_annotation: Type[Any],
) -> Tuple[Type[Any], List[ParameterInfo]]:
    if get_origin(base_annotation) is not Annotated:  # type: ignore
        return base_annotation, []
    base_annotation, *maybe_clix_annotations = get_args(base_annotation)
    return base_annotation, [
        annotation
        for annotation in maybe_clix_annotations
...
def get_params_from_function(func: Callable[..., Any]) -> Dict[str, ParamMeta]:
    ...
    for param in signature.parameters.values():
        annotation, clix_annotations = _split_annotation_from_clix_annotations(
            param.annotation,
        )
        if len(clix_annotations) > 1:
            raise MultipleClixAnnotationsError(param.name)

        default = param.default
        if clix_annotations:
            # It's something like `my_param: Annotated[str, Argument()]`
            [parameter_info] = clix_annotations
        ...
        if isinstance(default, ParameterInfo):
            ...
            # The two are not interchangeable for static typing, so clix allows
