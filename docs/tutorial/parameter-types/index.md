...
When you declare a *CLI parameter* with some type **Clix** will convert the data received in the command line to that data type.
...
And as `female` is a `bool` *CLI option*, **Clix** will convert it to a "flag" `--female` and the counterpart `--no-female`.
