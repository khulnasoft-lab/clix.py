...
Now that you also know how to use `clix.Argument()`, let's use it to add documentation specific for a *CLI argument*.
...
But you can customize it with the `metavar` parameter for `clix.Argument()`.
...
Nevertheless, **Clix supports `help` for *CLI arguments***. ✨ 🤷‍♂

**Clix** doesn't follow that convention and instead supports `help` to make it easier to have consistent help texts with a consistent format for your CLI programs. 🎨

This is also to help you create CLI programs that are ✨ awesome ✨ *by default*. With very little code.

If you want to keep Click's convention in a **Clix** app, you can do it with the `hidden` parameter as described above.

!!! note "Technical Details"
    To support `help` in *CLI arguments* **Clix** does a lot of internal work in its own sub-classes of Click's internal classes.
