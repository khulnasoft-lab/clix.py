...
It follows the same design and ideas. If you know **FastAPI**, you already know **Clix**... more or less.

## Just Modern Python

It's all based on standard **Python 3.6 type** declarations. No new syntax to learn. Just standard modern Python.

If you need a 2 minute refresher of how to use Python types (even if you don't use FastAPI or Clix), check the FastAPI tutorial section: <a href="https://fastapi.khulnasoft.com/python-types/" class="external-link" target="_blank">Python types intro</a>.
...
**Clix** was designed to be easy and intuitive to use, to ensure the best development experience. With autocompletion everywhere.
...
The resulting CLI apps created with **Clix** have the nice features of many "pro" command line programs you probably already love.
...
!!! note "* Auto completion"
    Auto completion works when you create a package (installable with `pip`). Or when using [Clix CLI](clix-cli.md){.internal-link target=_blank}.

    If you also add `shellingham` as a dependency, **Clix** will use it to auto-detect the current shell when installing completion.

    **Clix** will automatically create 2 *CLI options*:
...
!!! tip
    **Clix**'s completion is implemented internally, it uses ideas and components from Click and ideas from `click-completion`, but it doesn't use `click-completion` and re-implements some of the relevant parts of Click.

    Then it extends those ideas with features and bug fixes. For example, **Clix** programs also support modern versions of PowerShell (e.g. in Windows 10) among all the other shells.
...
**Clix** is based on it, so you get all its benefits, plug-ins, robustness, etc.
