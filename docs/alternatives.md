What inspired **Clix**, how it compares to other alternatives and what it learned from them.

## Intro

**Clix** wouldn't exist if not for the previous work of others.
...
It inspired a lot of the ideas in **FastAPI** and **Clix**.
...
!!! check "Inspired **Clix** to"
    Provide a simple way to use a function as a command line app, without having to create a complete app, with `clix.run(some_function)`.
...
It is not used by **Clix**, but it inspired a lot of the design (through **FastAPI**).
...
It's a very powerful tool and there are many CLIs built with it. It is what powers **Clix** underneath.
...
!!! check "**Clix** uses it for"
    Everything. 🚀

    **Clix** mainly adds a layer on top of Click, making the code simpler and easier to use, with autocompletion everywhere, etc, but providing all the powerful features of Click underneath.
...
Previous versions of **Clix** had deep integrations with `click-completion` and used it as an optional dependency. But now all the completion logic is implemented internally in **Clix** itself, the internal logic was heavily inspired and using some parts of `click-completion`.

And now **Clix** improved it to have new features, tests, some bug fixes (for issues in plain `click-completion` and Click), and better support for shells, including modern versions of PowerShell (e.g. the default versions that come with Windows 10).
...
**Clix** is the "FastAPI of CLIs".

It uses the same design and usage of FastAPI as much as possible. So, if you have used FastAPI, you know how to use Clix.
